...
// ── CSV Upload config ─────────────────────────────────────────────────────────
const OC_CSV_HEADERS = ['Vendor Code', 'Challan Date', 'WO No', 'Item Code', 'Qty', 'Unit', 'Notes'];
const OC_CSV_SAMPLE = [
  { 'Vendor Code': 'VND-001', 'Challan Date': '2025-06-15', 'WO No': '', 'Item Code': 'ITM-001', 'Qty': '100', 'Unit': 'pcs', 'Notes': '' },
];
const OC_CSV_VALIDATION = [
...
export default function OutwardChallanPage() {
  ...
  const canWrite = can('plan-subcontracting-outward_challan-create_edit_delete');
  ...
  const onSave = async () => {
    try {
      ...
      message.success('Outward challan created');
      setDrawerOpen(false);
      load();
    } catch (err) {
      ...
      render: (_, r) => {
        ...
        return (
          <Space size={4}>
            {isPending && (
              <Popconfirm
                title="Mark this challan as received?"
  ...
  return (
    <AppLayout>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
        ...
        <Text style={{ color: '#6b7280', fontSize: 12 }}>Outward Challan</Text>
      </div>

      <Title level={3} style={{ margin: 0 }}>Outward Challan</Title>
      <Text type="secondary" style={{ fontSize: 13 }}>
        Send materials to sub-contractors for processing.
      </Text>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          />
          <div style={{ flex: 1 }} />
          <Button icon={<DownloadOutlined />} onClick={() => exportTableToCsv('outward-challan.csv', challans, columns)}>Export CSV</Button>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="notes" label="Notes">
            <Input.TextArea rows={2} placeholder="Instructions for sub-contractor…" />
          </Form.Item>
        ...
        {detailLoading ? (
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px', marginBottom: 20 }}>
              ...
              <div>
                <Text type="secondary" style={{ fontSize: 11 }}>Challan Date</Text>
      ...
      <CsvUploadModal
        ...
        title="Upload Outward Challans"
        entityName="Outward Challan"
        sampleHeaders={OC_CSV_HEADERS}
        sampleRows={OC_CSV_SAMPLE}
        validationRules={OC_CSV_VALIDATION}
      />
    </AppLayout>
  );
}
